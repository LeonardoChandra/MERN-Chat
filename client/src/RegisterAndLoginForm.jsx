  ...
  return (
    <div className="bg-blue-50 h-screen flex items-center">
      <form className="w-64 mx-auto" onSubmit={handleSubmit}>
        ...
        <button className="bg-blue-500 text-white w-full block rounded-sm p-2">{isLoginOrRegister === "register" ? "Register" : "Login"}</button>
        <div className="text-center mt-2">
          {isLoginOrRegister === "register" && (
            <div>
              Have an account already? <button onClick={() => setIsLoginOrRegister("login")}>Login</button>
            </div>
          )}
